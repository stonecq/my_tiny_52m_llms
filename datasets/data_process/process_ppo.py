...
def process_line(line):
    try:
        ...
        # 拼接
        full_chosen_ids = prompt_ids + [tokenizer.special_tokens['<bos>']]
        return full_chosen_ids
    except:
        return None


def preprocess_to_pickle(input_file, output_pkl):
    tokenizer_path = "/llama/work_space/my_llms_learn/my_tiny_llms/tokenizer/chatglm"

    with open(input_file, 'rb') as f:
        lines = f.readlines()

    lines = random.sample(lines, num_data_from_rlhf_data)
...
if __name__ == "__main__":
    preprocess_to_pickle("/llama/datasets/tiny_llms/rl_train/rl_train_data.jsonl",
                         "/llama/datasets/tiny_llms/rl_train/ppo_data.pkl")